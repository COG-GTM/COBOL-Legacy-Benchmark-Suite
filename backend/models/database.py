from sqlalchemy import (
    create_engine, Column, String, Numeric, Date, DateTime,
    CheckConstraint, ForeignKeyConstraint, Index,
)
...
class Position(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(["portfolio_id"], ["portfolios.port_id"]),
    ...
    def calculate_gain_loss(self) -> Dict:
        ...
        gain_loss_percent = (
            (gain_loss / self.cost_basis) * 100
            if self.cost_basis != 0
            else Decimal("0.00")
        )
        return {"gain_loss": gain_loss, "gain_loss_percent": gain_loss_percent}

    def to_dict(self) -> Dict:
        gl = self.calculate_gain_loss()
        return {
            ...
            "gain_loss_percent": float(gl["gain_loss_percent"]),
        }
