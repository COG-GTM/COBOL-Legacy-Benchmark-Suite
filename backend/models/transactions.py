from sqlalchemy import (
    Column, String, Numeric, Date, Time, DateTime,
...
class Transaction(Base):
    ...
    def to_dict(self) -> Dict:
        return {
            ...
            "type": self.type,
            "typeName": self.TYPE_LABELS.get(self.type, self.type),
            "quantity": float(self.quantity) if self.quantity else 0.0,
            "price": float(self.price) if self.price else 0.0,
            "amount": float(self.amount) if self.amount else 0.0,
            "currency": self.currency,
            "status": self.status,
            "statusName": self.STATUS_LABELS.get(self.status, self.status),
            "processDate": self.process_date.isoformat() if self.process_date else None,
