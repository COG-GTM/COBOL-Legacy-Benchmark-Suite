...
/**
 * Adds two decimal strings exactly, using scaled BigInt arithmetic to avoid
 * IEEE-754 rounding. The result keeps the wider of the two inputs' fraction
 * digits. Assumes both inputs are valid signed decimal strings.
 */
export function addDecimals(a: string, b: string): string {
  const fracDigits = Math.max(fractionDigits(a), fractionDigits(b));
  const sum = toScaledInt(a, fracDigits) + toScaledInt(b, fracDigits);
  return fromScaledInt(sum, fracDigits);
}
...
function toScaledInt(value: string, fracDigits: number): bigint {
  const trimmed = value.trim();
  const negative = trimmed.startsWith('-');
  const unsigned = trimmed.replace(/^[-+]/, '');
  const [intPart, fracPart = ''] = unsigned.split('.');
  const scaled = `${intPart}${fracPart.padEnd(fracDigits, '0')}`;
  const magnitude = BigInt(scaled || '0');
  return negative ? -magnitude : magnitude;
}

function fromScaledInt(scaled: bigint, fracDigits: number): string {
  const negative = scaled < 0n;
  const digits = (negative ? -scaled : scaled).toString().padStart(fracDigits + 1, '0');
  const intPart = digits.slice(0, digits.length - fracDigits) || '0';
  const fracPart = fracDigits > 0 ? digits.slice(digits.length - fracDigits) : '';
  const body = fracDigits > 0 ? `${intPart}.${fracPart}` : intPart;
