...
| BR-03 | `TRN-TYPE` must be one of `BU`, `SL`, `TR`, `FE`; otherwise `Invalid Transaction Type: <type>`, with the rejected value strung into the message | `2120-CHECK-TRANSACTION-TYPE` | `TransactionType` enum + `TransactionValidator.checkTransactionType(String)` | `TransactionValidatorTest.unknownTypeRejected`, `PortfolioTransactionControllerTest.invalidTypeCarriesTheOffendingValue` |
...
| BR-10 | `SL`: reject with `Insufficient units for sale` when `PORT-TOTAL-UNITS < TRN-QUANTITY`, otherwise subtract units and cost. The COBOL reads the position from `PORTFILE`, so an *absent* position is a caller error (`400`), not this business rejection | `2220-PROCESS-SELL` | `TransactionPostingService.processSell` | `TransactionPostingServiceTest.sellSubtractsUnitsAndCost`, `sellRejectedWhenUnitsInsufficient`, `sellExactlyAvailableUnits` |
...
| BR-13 | Audit action per type: `BU → CREATE`, `SL → DELETE`, `TR → UPDATE`, `FE → UPDATE`. `2200-UPDATE-POSITIONS` performs `2300-UPDATE-AUDIT-TRAIL` unconditionally, so a *rejected* posting is still audited, with `AUD-STATUS = 'FAIL'` instead of `'SUCC'`. A record rejected earlier, in `2100-VALIDATE-TRANSACTION`, branches to `9000` and is never audited | `2200-UPDATE-POSITIONS`, `2300-UPDATE-AUDIT-TRAIL` | `TransactionType.getAuditAction`, `TransactionProcessingResult.postingSuccess` / `postingFailure` / `validationFailure` | `TransactionPostingServiceTest.auditActionMapping`, `PortfolioTransactionServiceTest.failedPostingKeepsAuditAction`, `validationFailureHasNoAuditEntry`, `PortfolioTransactionControllerTest.failedPostingReportsAuditEntry` |
...
| # | Rule | COBOL paragraph | Java | Test |
| --- | --- | --- | --- | --- |
| BR-20 | Sequence numbers are assigned 1, 2, 3 … in read order and zero filled into `TRN-SEQUENCE-NO PIC X(06)`; the next value for a date + portfolio is the highest existing value + 1. The single-threaded batch cannot race, so the service derives and writes the number inside one transaction and re-numbers when a concurrent writer took it | `1200-BUILD-SEQUENCE`, `1210-ADD-TO-SEQUENCE` | `PortfolioTransactionService.insertNextInSequence`, `TransactionSequenceService.nextSequenceNo` | `PortfolioTransactionServiceTest.sequenceAssignment`, `insertAssignsSequenceNumber`, `insertRetriesWhenTheSequenceNumberIsTaken`, `PortfolioTransactionRepositoryTest.maxSequenceNoPerDateAndPortfolio` |
...
| BR-23 **(derived)** | `TRN-STATUS` transitions: `P → D`, `P → F`, `D → R`; `F` and `R` are terminal. Processing writes `D` or `F`, so it is refused for a record that is no longer pending — a finished record cannot be posted to the portfolio a second time | The programs never assign `TRN-STATUS`; the model follows how `2100-VALIDATE-TRANSACTION` classifies records as processed or failed | `TransactionStatus.allowedTransitions`, `PortfolioTransactionService.process` | `TransactionStatusTest`, `PortfolioTransactionServiceTest.statusTransitions`, `terminalRecordsCannotBeReprocessed`, `PortfolioTransactionControllerTest.processRefusedOnceTerminal` |
...
| OQ-2 | The portfolio existence check (BR-02) reads the portfolio master, which is a different entity and out of scope for this slice. Should this service call a portfolio service, or is a format check enough? And where should `PORT-TOTAL-UNITS` come from for an `SL` posting — today the caller supplies it? | `PortfolioReferenceValidator` interface; the default implementation applies the `PORTVALD 1000-VALIDATE-ID` format rule only. A deployment that owns the portfolio table sets `portfolio.reference-validator=external` and contributes its own bean. `availableUnits` is a required input for `SL` and its absence is a `400`, never a BR-10 rejection. | `FormatOnlyPortfolioReferenceValidator`, `TransactionMigrationConfiguration`, `TransactionPostingService.processSell` |
...
| OQ-10 | `TRN-QUANTITY × TRN-PRICE` can exceed the 15 digits of `TRN-AMOUNT S9(13)V9(2)`; the COBOL would truncate the high-order digits silently (a `SIZE ERROR` phrase is nowhere in the supplied source). Should the service replicate that, or refuse the record? | The service refuses: the multiplication result is checked against `DECIMAL(15,2)` and an out-of-range product is rejected rather than stored with high-order digits dropped. Silent corruption of a money field is not a behaviour worth preserving without the owners' confirmation. | `TransactionAmountCalculator.computeAmount` |
| OQ-11 | `TRN-DATE X(08)` and `TRN-TIME X(06)` are character fields, so `20240230` or `99999999` fits the copybook and the batch never parses them. Should the service reject calendar-invalid keys? | The key stores and orders the raw characters, exactly as VSAM does. The typed views `getTransactionDate()` / `getTransactionTime()` return `Optional`, empty for a value that is not a real date or time, so no read path can throw on legacy data. | `TransactionKey.getTransactionDate` |
