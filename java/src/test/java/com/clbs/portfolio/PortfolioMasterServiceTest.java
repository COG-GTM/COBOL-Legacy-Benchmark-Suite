...
class PortfolioMasterServiceTest {
    ...
    void createsThenReadsPortfolio() {
        assertThat(service.create(record("PORT0001", "12345678", "GROWTH", 'A')).ok()).isTrue();

        PortfolioMasterService.Response read = service.read("PORT0001", "12345678");
    ...
    void rejectsDuplicateCreate() {
        service.create(record("PORT0001", "12345678", "GROWTH", 'A'));
    ...
    void rejectsMissingNameAndBadStatus() {
        assertThat(service.create(record("PORT0001", "12345678", "  ", 'A')).errorText())
                .isEqualTo(PortfolioMasterService.ERR_NAME_REQUIRED);
        assertThat(service.create(record("PORT0002", "12345678", "GROWTH", 'S')).errorText())
    ...
    void updateRequiresExistingRecord() {
        PortfolioMasterService.Response response =
                service.update(record("PORT0009", "12345678", "GROWTH", 'A'));
    ...
    void updatesAndDeletesExistingRecord() {
        ...
        PortfolioRecord updated = record("PORT0001", "12345678", "INCOME", 'I');
        assertThat(service.update(updated).ok()).isTrue();
        assertThat(service.read("PORT0001", "12345678").record().getClientName())
                .isEqualTo("INCOME");

        assertThat(service.delete("PORT0001", "12345678").ok()).isTrue();
        assertThat(service.read("PORT0001", "12345678").errorText())
    ...
    void unknownCommandIsRejected() {
        PortfolioMasterService.Response response =
                service.execute('X', record("PORT0001", "12345678", "GROWTH", 'A'));
        assertThat(response.errorText()).isEqualTo(PortfolioMasterService.ERR_INVALID_COMMAND);
    }

    @Test
    void totalValueKeepsTwoDecimalScale() {
        PortfolioRecord record = record("PORT0001", "12345678", "GROWTH", 'A');
        record.setTotalValue(new BigDecimal("1234.5"));
        service.create(record);

        assertThat(service.read("PORT0001", "12345678").record().getTotalValue())
